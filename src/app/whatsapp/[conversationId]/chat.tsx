...
import { ArrowDownCircle, Reply } from "lucide-react";
    ...
    <div>
...

      {
        conversation.messages.map((message, index) => {
          ...
          const messageQuoted= conversation.messages.find(m => m.wapId === quotedId)
        ...
        return (
          message.role === "user" ?
          <>
            <div key={index} className="max-w-sm p-2 mt-3 mb-2 bg-white rounded-lg xl:max-w-lg">
          ...
          <>
            <div key={index} className="bg-[#dcf8c6] rounded-lg p-2 mt-3 mb-2 max-w-lg self-end min-w-[384px]">
              <div className="flex justify-between">
                <p className="text-xs text-[#00a884]">{message.name}</p>
                <Button variant="secondary" className="h-5 gap-2 px-1" onClick={() => reply(message)}>
                  <Reply className="text-muted-foreground" />
...

              {
                ...
                <>
                  ...
                  <p className="text-xs text-right">{format(message.createdAt, "HH:mm")}</p>
                </>
              }
            </div>
      ...
      <div className="my-3">
          <SendText conversationId={conversation.id} replayId={replyId} replyName={replayName} replayText={replayText} type={replayType} notifyCloseReplay={notifyCloseReplay} />
      </div>
    </div>
  )
}
