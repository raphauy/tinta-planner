...
import { deleteConversationAction, createOrUpdateConversationAction, getConversationDAOAction } from "./conversation-actions"
import { conversationSchema, ConversationFormValues } from '@/services/conversation-services'
...
export function ConversationForm({ id, closeDialog }: Props) {
  ...
  const onSubmit = async (data: ConversationFormValues) => {
    setLoading(true)
    try {
      await createOrUpdateConversationAction(id ? id : null, data)
      toast({ title: id ? "Conversation updated" : "Conversation created" })
      closeDialog()
    } catch (error: any) {
